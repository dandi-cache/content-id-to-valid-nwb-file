...
This one also has `code/refresh.py`, a second entry point declared in `cache.toml` and selected by the workflow that runs it.
It re-assesses what the update already recorded, so it is the same operation over a different batch and calls into `code/update.py` rather than repeating any of it.
...
- Only what is genuinely specific to this cache's operation belongs in `code/update.py`, `code/refresh.py` or `cache.toml`.
...
- Locally: `uv run --project envs --extra local python code/update.py --testing`, and the same for `code/refresh.py`.
- Then through the real pipeline: dispatch `Update` with `testing: true` and confirm it writes `derivatives/testing_content_id_to_valid_nwb_file.jsonl` alongside the two `testing_`-prefixed files that accompany it, and leaves the real cache untouched.
  Dispatch `Refresh` the same way: it exercises the other batch selection over the same operation.
